...
async def create_chat_completion(
    ...
    # FORCE USE GEMINI AS PRIMARY PROVIDER (OpenAI fallback disabled)
    # Override llm_provider to use Gemini directly
    if llm_provider == "openai" or llm_provider is None:
        llm_provider = "gemini"
        logging.getLogger(__name__).info("🔄 OpenAI provider overridden to Gemini as primary provider")
    
    # Set default model for Gemini if not specified
    if model is None or model.startswith("gpt-"):
        model = "gemini-2.0-flash"
        logging.getLogger(__name__).info(f"🔄 Model overridden to {model} for Gemini provider")
    ...
    # OpenAI base URL configuration is now bypassed since we're forcing Gemini
    # Keeping the code but it won't execute as llm_provider is now "gemini"
    ...
    # create response with retry mechanism (OpenAI fallback completely disabled)
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            # OPENAI FALLBACK COMPLETELY DISABLED - Using only Gemini
            # The fallback code is commented out to prevent any OpenAI usage
            
            # Retry logic for attempts
            if attempt < max_attempts:
                await asyncio.sleep(min(2 ** (attempt - 1), 8))
                continue
            break
